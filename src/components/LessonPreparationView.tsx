                            ...
                            {currentProjectorItem.type === 'leitura' ? (
                              <>
                                {currentProjectorItem.reference && (
                                  <h3 className="text-xl md:text-3xl font-black text-yellow-400 tracking-wide font-sans text-center mb-1 w-full">
                                    {currentProjectorItem.reference}
                                  </h3>
                                )}
                                <div className="w-full space-y-2 text-left font-sans max-h-[260px] overflow-y-auto pr-1">
                                  {(currentProjectorItem.projetorText || '').split('\n').filter(l => l.trim()).map((line, idx) => {
                                    const match = line.match(/^(\d{1,3})\s*(?:[—\-–\.]\s*)?(.+)$/);
                                    if (match) {
                                      return (
                                        <div key={idx} className="flex items-start gap-2 text-left w-full py-1 border-b border-slate-200/40 last:border-0">
                                          <span className="shrink-0 font-black text-white text-xl md:text-2xl leading-none mt-0.5">{match[1]}</span>
                                          <p className="font-bold text-white text-sm md:text-base leading-snug break-words flex-1">{match[2]}</p>
                                        </div>
                                      );
                                    }
                                    return <p key={idx} className="font-bold text-white text-sm md:text-base leading-snug break-words text-left">{line}</p>;
                                  })}
                                </div>
                              </>
                            ) : currentProjectorItem.type === 'conclusao' || currentProjectorItem.type === 'topic_synopsis' ? (
                              <p className="text-lg md:text-xl font-extrabold leading-relaxed text-white text-center font-sans break-words">
                              ...
                              <div className="w-full space-y-2 text-left font-sans">
                                {currentProjectorItem.bulletPoints?.map((point, idx) => (
                                  <div key={idx} className="flex items-start gap-2 py-1">
                                    ...
                                    <p className="text-sm md:text-base font-bold leading-snug text-white font-sans break-words">{point}</p>
                                  </div>
                                ))}
                              </div>
                            ) : (
